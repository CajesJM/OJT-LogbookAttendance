...
function isRateLimited(key: string) {
  ...
  const attempts = (recentRequests.get(key) ?? []).filter((time) => time > windowStart);
...
function validatePayload(payload: FeedbackPayload) {
  ...
  if (!(kind in KIND_LABELS)) return { error: "Choose a valid feedback type." } as const;
  if (subject.length < 4 || subject.length > 80) return { error: "The title must contain 4 to 80 characters." } as const;
  if (details.length < 15 || details.length > 1000) return { error: "The details must contain 15 to 1000 characters." } as const;
  if (email && !isEmail(email)) return { error: "Enter a valid reply email address." } as const;

  let attachment: { content: string; filename: string } | undefined;
  if (payload.attachment) {
    const { content, filename, contentType, size } = payload.attachment;
    if (typeof content !== "string" || typeof filename !== "string" || typeof contentType !== "string" || typeof size !== "number") {
      return { error: "The screenshot is invalid." } as const;
    }
    if (!ALLOWED_ATTACHMENT_TYPES.has(contentType) || size <= 0 || size > MAX_ATTACHMENT_BYTES) {
      return { error: "Use a JPG, PNG, or WebP screenshot smaller than 2 MB." } as const;
    }
    const expectedBase64Length = Math.ceil(size / 3) * 4;
    if (!/^[A-Za-z0-9+/]*={0,2}$/.test(content) || Math.abs(content.length - expectedBase64Length) > 4) {
  ...
  return {
    data: { kind: kind as FeedbackKind, subject, details, email, pageUrl, attachment },
  } as const;
}
...
export default {
  async fetch(request: Request) {
    ...
    if (typeof payload.startedAt !== "number" || Date.now() - payload.startedAt < 1200) {
      return jsonResponse({ error: "Please review your feedback before sending it." }, 400);
    }
    if (isRateLimited(getClientKey(request))) {
      return jsonResponse({ error: "Too many feedback messages. Please try again in 10 minutes." }, 429);
    }
    ...
    const sender = process.env.RESEND_FROM_EMAIL || "OJT Logbook <onboarding@resend.dev>";
    if (!apiKey || !recipient) {
      console.error("Feedback email is missing RESEND_API_KEY or FEEDBACK_TO_EMAIL.");
      return jsonResponse({ error: "Feedback email is not configured yet." }, 503);
    }

    const { kind, subject, details, email, pageUrl, attachment } = validation.data;
    ...
    const resendResponse = await fetch(RESEND_ENDPOINT, {
      ...
      headers: {
        ...
        ...(submissionId ? { "Idempotency-Key": `feedback-${submissionId}` } : {}),
      },
      body: JSON.stringify({
        from: sender,
        to: [recipient],
    ...
    if (!resendResponse.ok) {
      ...
      console.error(`Resend feedback request failed (${resendResponse.status}): ${resendError.slice(0, 500)}`);
      return jsonResponse({ error: "Email delivery failed. Please try again shortly." }, 502);
    }
