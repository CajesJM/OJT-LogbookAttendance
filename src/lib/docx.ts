...
function cell(content: string, width: number, align?: "right" | "center") {
  ...
  const body = content.startsWith("<w:p>") ? content : paragraph(content, alignment);
...
function zip(entries: ZipEntry[]) {
  ...
  entries.forEach(({ name, data }) => {
    ...
    write16(localView, 26, nameBytes.length); local.set(nameBytes, 30);
    localParts.push(local, data);

    const central = new Uint8Array(46 + nameBytes.length);
    const centralView = new DataView(central.buffer);
    write32(centralView, 0, 0x02014b50); write16(centralView, 4, 20); write16(centralView, 6, 20);
    write16(centralView, 8, 0x0800); write32(centralView, 16, checksum); write32(centralView, 20, data.length);
    write32(centralView, 24, data.length); write16(centralView, 28, nameBytes.length); write32(centralView, 42, offset);
  ...
  write32(endView, 12, centralDirectory.length); write32(endView, 16, offset);
  return joinBytes([...localParts, centralDirectory, end]);
}

function saveDocx(documentXml: string, relationships: string[], images: ZipEntry[], fileName: string) {
...
export async function downloadOjtReportDocx({ user, profile, records, separateByMonth = false, template = "detailed" }: DocxReportData) {
  ...
  const groups = entries.reduce<Array<{ key: string; label: string | null; entries: typeof entries }>>((result, entry) => {
    const key = separateByMonth ? entry.record.date.slice(0, 7) : "all";
    const date = new Date(`${entry.record.date}T00:00:00`);
    const label = separateByMonth ? date.toLocaleDateString("en-US", { month: "long", year: "numeric" }) : null;
    const current = result[result.length - 1];
    if (!current || current.key !== key) result.push({ key, label, entries: [entry] });
  ...
  if (template === "worklog") {
    ...
    const worklogSignatures = new Map<string, { relationshipId: string; imageId: number }>();
    let worklogImageId = 1;
    entries.forEach(({ record }) => {
      if (!record.signature) return;
      const relationshipId = `rId${worklogImageId}`;
      worklogImages.push({ name: `word/media/signature-${worklogImageId}.png`, data: dataUrlBytes(record.signature) });
      worklogRelationships.push(`<Relationship Id="${relationshipId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/signature-${worklogImageId}.png"/>`);
      worklogSignatures.set(record.id, { relationshipId, imageId: worklogImageId });
      worklogImageId += 1;
    });
    const worklogGroups = groups.map((group, groupIndex) => {
      const pageBreak = separateByMonth && groupIndex > 0 ? paragraph("<w:r><w:br w:type=\"page\"/></w:r>") : "";
      ...
      const taskWidth = 9800 - dateWidth - timeWidth - remainingWidth - noteWidth - signatureWidth;
      const widths = [dateWidth, taskWidth, timeWidth, remainingWidth, ...(showNote ? [noteWidth] : []), ...(showSignature ? [signatureWidth] : [])];
      ...
      const rows = group.entries.map(({ record, remaining }) => {
        ...
        const signatureXml = signature ? signatureDrawing(signature.relationshipId, signature.imageId) : "";
    ...
    saveDocx(worklogDocument, worklogRelationships, worklogImages, `ojt-work-log-${new Date().toISOString().slice(0, 10)}.docx`);
    return;
  }
  ...
  let imageId = 1;
  const recordSignature = new Map<string, { relationshipId: string; imageId: number }>();
  entries.forEach(({ record }) => {
    if (!record.signature) return;
    const relationshipId = `rId${imageId}`;
    images.push({ name: `word/media/signature-${imageId}.png`, data: dataUrlBytes(record.signature) });
    relationships.push(`<Relationship Id="${relationshipId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/signature-${imageId}.png"/>`);
    recordSignature.set(record.id, { relationshipId, imageId });
    imageId += 1;
  });

  const summary = [
    ["Name", profile.fullName || user.name], ["Gmail", profile.email || user.email || "Not set"],
    ["School", profile.school || "Not set"], ["Course", profile.course || "Not set"],
    ["Company", profile.companyName || "Not set"], ["Department", profile.department || "Not set"],
    ["Supervisor", profile.supervisorName || "Not set"], ["Start date", formatDate(profile.ojtStartDate)],
    ["Rendered hours", formatHours(totalHours)], ["Required hours", formatHours(requiredHours)],
  ];
  ...
  const groupXml = groups.map((group, groupIndex) => {
    const pageBreak = separateByMonth && groupIndex > 0 ? paragraph("<w:r><w:br w:type=\"page\"/></w:r>") : "";
    const heading = paragraph(textRun(`Daily Records${group.label ? ` - ${group.label}` : ""}`, true, 23), "<w:spacing w:before=\"160\" w:after=\"80\"/>");
    ...
    const rows = group.entries.map(({ record, remaining }) => {
      const details = [
        ...
        ...(record.reflection ? [paragraph(textRun("Reflection: ", true, 16) + textRun(record.reflection, false, 16))] : []),
      ].join("");
  ...
  saveDocx(documentXml, relationships, images, `ojt-logbook-${new Date().toISOString().slice(0, 10)}.docx`);
}
